...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName("RatingsStreamingJob").getOrCreate()
    ...
    checkpoint_config = CheckpointConfig(
        checkpoint_path=os.environ.get(
            "CHECKPOINT_PATH", "s3a://rideshare-checkpoints/ratings/"
        ),
        trigger_interval=os.environ.get("TRIGGER_INTERVAL", "10 seconds"),
    )
    error_handler = ErrorHandler(dlq_table_path="s3a://rideshare-bronze/dlq_ratings/")

    job = RatingsStreamingJob(spark, kafka_config, checkpoint_config, error_handler)
    query = job.start()
