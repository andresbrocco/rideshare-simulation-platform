"""
Bronze Driver Status Streaming - Databricks Notebook

Driver status change event data streaming from Kafka to Delta Lake.
Captures driver status transitions (available, on_trip, offline, etc.).

Events Handled:
- driver.status_changed: Driver status transition events with previous and current status
...
TABLE_NAME = "bronze_driver_status"
TOPIC_NAME = "driver-status"
CONSUMER_GROUP = "databricks-bronze-consumer"
MAX_OFFSETS_PER_TRIGGER = 5000
TRIGGER_INTERVAL = "30 seconds"


def get_driver_status_schema():
    """
    Get schema for bronze_driver_status table.
...
def get_partition_columns():
    """Get partition columns for driver status table."""
    return ["ingestion_date"]


def get_write_mode():
    """Get write mode for driver status table."""
    return "append"


def create_bronze_driver_status_table(spark):
    """
    Create bronze_driver_status table if it doesn't exist.
...
def setup_driver_status_stream(spark):
    """
    Set up driver status streaming from Kafka to Delta Lake.

    Configures:
    - Driver-status topic consumption (8 partitions, partitioned by driver_id)
    ...
    # Get checkpoint path (unique for driver-status)
    checkpoint_path = create_checkpoint_path(TOPIC_NAME)

    # Write to Delta Lake
    query = (
        status_df.writeStream.format("delta")
        .outputMode("append")
        .option("checkpointLocation", checkpoint_path)
        .partitionBy("ingestion_date")
        .trigger(processingTime=TRIGGER_INTERVAL)
        .queryName("bronze_driver_status")
        .toTable(f"{CATALOG}.{SCHEMA}.{TABLE_NAME}")
    )

    print("Started streaming query: bronze_driver_status")
...
#
# # Start driver status stream
# query = setup_driver_status_stream(spark)
#
