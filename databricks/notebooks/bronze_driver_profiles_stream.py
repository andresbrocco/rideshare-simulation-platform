"""
Bronze Driver Profiles Streaming - Databricks Notebook

Low-volume driver profile data streaming from Kafka to Delta Lake.
Handles profile creation and update events.

Events Handled:
- driver.created: Initial profile creation with all fields
- driver.updated: Profile updates (vehicle, preferences, contact info)
...
TABLE_NAME = "bronze_driver_profiles"
TOPIC_NAME = "driver-profiles"
...
def get_driver_profiles_schema():
    """
    Get schema for bronze_driver_profiles table.
...
def get_partition_columns():
    """Get partition columns for driver profiles table."""
...
def create_bronze_driver_profiles_table(spark):
    """
    Create bronze_driver_profiles table if it doesn't exist.
...
def setup_driver_profiles_stream(spark):
    """
    Set up driver profiles streaming from Kafka to Delta Lake.
    ...
    # Get checkpoint path (unique for driver-profiles)
    checkpoint_path = create_checkpoint_path(TOPIC_NAME)

    # Write to Delta Lake
    query = (
        driver_profiles_df.writeStream.format("delta")
        .outputMode("append")
        .option("checkpointLocation", checkpoint_path)
        .partitionBy("ingestion_date")
        .trigger(processingTime=TRIGGER_INTERVAL)
        .queryName("bronze_driver_profiles")
        .toTable(f"{CATALOG}.{SCHEMA}.{TABLE_NAME}")
    )

    print("Started streaming query: bronze_driver_profiles")
...
#
# # Start driver profiles stream
# query = setup_driver_profiles_stream(spark)
#
