"""
Bronze Payments Streaming - Databricks Notebook

Payment event data streaming from Kafka to Delta Lake.
Captures payment processing events for completed trips.

Events Handled:
- payment.processed: Payment transaction with amount, method, and status
...
def get_payments_schema():
    """
    Get schema for bronze_payments table.
...
def get_partition_columns():
    """Get partition columns for payments table."""
    return ["ingestion_date"]


def get_write_mode():
    """Get write mode for payments table."""
    return "append"


def create_bronze_payments_table(spark):
    """
    Create bronze_payments table if it doesn't exist.
...
def setup_payments_stream(spark):
    """
    Set up payments streaming from Kafka to Delta Lake.

    Configures:
    - Payments topic consumption (8 partitions, partitioned by trip_id)
    ...
    # Get checkpoint path (unique for payments)
    checkpoint_path = create_checkpoint_path(TOPIC_NAME)

    # Write to Delta Lake
    query = (
        payments_df.writeStream.format("delta")
        .outputMode("append")
        .option("checkpointLocation", checkpoint_path)
        .partitionBy("ingestion_date")
        .trigger(processingTime=TRIGGER_INTERVAL)
        .queryName("bronze_payments")
        .toTable(f"{CATALOG}.{SCHEMA}.{TABLE_NAME}")
    )

    print("Started streaming query: bronze_payments")
...
# query = setup_payments_stream(spark)
#
