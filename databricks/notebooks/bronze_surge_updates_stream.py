"""
Bronze Surge Updates Streaming - Databricks Notebook

Surge pricing update event data streaming from Kafka to Delta Lake.
Captures zone-level surge multiplier recalculations.

Events Handled:
- surge.updated: Zone surge multiplier updates with supply/demand metrics
...
TABLE_NAME = "bronze_surge_updates"
TOPIC_NAME = "surge-updates"
CONSUMER_GROUP = "databricks-bronze-consumer"
MAX_OFFSETS_PER_TRIGGER = 2000
TRIGGER_INTERVAL = "60 seconds"  # Matches surge recalculation frequency


def get_surge_updates_schema():
    """
    Get schema for bronze_surge_updates table.
...
def get_partition_columns():
    """Get partition columns for surge updates table."""
    return ["ingestion_date"]


def get_write_mode():
    """Get write mode for surge updates table."""
    return "append"


def create_bronze_surge_updates_table(spark):
    """
    Create bronze_surge_updates table if it doesn't exist.
...
def setup_surge_updates_stream(spark):
    """
    Set up surge updates streaming from Kafka to Delta Lake.

    Configures:
    - Surge-updates topic consumption (8 partitions, partitioned by zone_id)
    - Backpressure via maxOffsetsPerTrigger (2000)
    - Partition by ingestion_date
    - Distributed tracing field extraction
    - 60-second trigger interval (matches surge recalculation)
    ...
    # Get checkpoint path (unique for surge-updates)
    checkpoint_path = create_checkpoint_path(TOPIC_NAME)

    # Write to Delta Lake
    query = (
        surge_df.writeStream.format("delta")
        .outputMode("append")
        .option("checkpointLocation", checkpoint_path)
        .partitionBy("ingestion_date")
        .trigger(processingTime=TRIGGER_INTERVAL)
        .queryName("bronze_surge_updates")
        .toTable(f"{CATALOG}.{SCHEMA}.{TABLE_NAME}")
    )

    print("Started streaming query: bronze_surge_updates")
...
#
# # Start surge updates stream
# query = setup_surge_updates_stream(spark)
#
