"""
Bronze Rider Profiles Streaming - Databricks Notebook

Low-volume rider profile data streaming from Kafka to Delta Lake.
Handles profile creation and update events.

Events Handled:
- rider.created: Initial profile creation with behavior_factor (DNA)
- rider.updated: Profile updates (contact info, payment method)
  Note: behavior_factor is immutable and only appears in rider.created
...
TABLE_NAME = "bronze_rider_profiles"
TOPIC_NAME = "rider-profiles"
CONSUMER_GROUP = "databricks-bronze-consumer"

# Low-volume configuration
TRIGGER_INTERVAL = "60 seconds"  # Less frequent for low volume


def get_rider_profiles_schema():
    """
    Get schema for bronze_rider_profiles table.
...
def get_partition_columns():
    """Get partition columns for rider profiles table."""
    return ["ingestion_date"]


def get_trigger_interval():
    """Get trigger interval for low-volume streaming."""
    return TRIGGER_INTERVAL


def create_bronze_rider_profiles_table(spark):
    """
    Create bronze_rider_profiles table if it doesn't exist.
...
def setup_rider_profiles_stream(spark):
    """
    Set up rider profiles streaming from Kafka to Delta Lake.

    Configures:
    - Low-volume trigger interval (60 seconds)
    ...
    kafka_stream = kafka_stream.option("failOnDataLoss", "false").option(
    ...
    rider_profiles_df = (
        ...
        .withColumn("event_type", F.get_json_object(F.col("value"), "$.event_type"))
    ...
    # Get checkpoint path (unique for rider-profiles)
    checkpoint_path = create_checkpoint_path(TOPIC_NAME)

    # Write to Delta Lake
    query = (
        rider_profiles_df.writeStream.format("delta")
        .outputMode("append")
        .option("checkpointLocation", checkpoint_path)
        .partitionBy("ingestion_date")
        .trigger(processingTime=TRIGGER_INTERVAL)
        .queryName("bronze_rider_profiles")
        .toTable(f"{CATALOG}.{SCHEMA}.{TABLE_NAME}")
    )

    print("Started streaming query: bronze_rider_profiles")
...
#
# # Start rider profiles stream
# query = setup_rider_profiles_stream(spark)
#
