...
- **Owns**: AWS resource lifecycle (VPC, EKS, RDS, S3 buckets, IAM roles, ACM certs, CloudFront, ECR, Glue catalog databases, Lambda function, Secrets Manager secrets, Route 53 zones and records)
...
2. `foundation` — long-lived shared resources: VPC, subnets, security groups, Route 53 zone, ACM certificate, S3 lakehouse buckets, ECR repositories, Secrets Manager secrets, IAM roles (both EKS cluster/node roles and per-workload Pod Identity roles), Glue catalog databases, and the `rideshare-auth-deploy` Lambda.
...
- The `rideshare-auth-deploy` Lambda uses `authorization_type = "NONE"` on its function URL — auth is implemented inside the function itself (API key + GitHub PAT validation), not at the AWS layer.
