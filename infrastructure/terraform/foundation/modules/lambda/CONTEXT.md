...
- **Owns**: Lambda function resource, execution IAM role, CloudWatch log group (with retention), Function URL with CORS, CloudWatch alarms for errors and throttles
...
- **Conditional IAM policies**: Policies for Secrets Manager (`secrets_arns`), SSM Parameter Store (`ssm_parameter_arns`), and EventBridge Scheduler (`scheduler_config`) are only created when the caller supplies non-empty values. This keeps the IAM surface minimal by default.
