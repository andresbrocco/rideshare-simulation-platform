...
- **Two Trino password-hash secrets are not managed by Terraform**: `{project}/trino-admin-password-hash` is created by the deploy workflow after the admin password is established; `{project}/trino-visitor-password-hash` is created on demand by the provisioning Lambda when the first visitor signs up. Neither secret exists in Terraform state — do not attempt to import or manage them here.
