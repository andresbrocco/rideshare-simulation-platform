...
**Secret groups**: All credentials are stored as JSON objects under `rideshare/*` in Secrets Manager. `infrastructure/scripts/seed-secrets.py` populates LocalStack for local development. Secret group names: `rideshare/api-key`, `rideshare/core` (includes Grafana credentials), `rideshare/data-pipeline`, `rideshare/github-pat`.
