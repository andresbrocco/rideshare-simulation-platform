...
- **Trino admin password hash computed at pod startup**: The `setup-config` initContainer (image: `httpd:2.4-alpine`) reads `ADMIN_PASSWORD` from the `app-credentials` Kubernetes Secret (synced from the `data-pipeline` secret by ESO) and computes the bcrypt hash via `htpasswd`. Only the `admin` account is defined. The rendered `password.db` is written to a shared `emptyDir` volume with `chmod 600` — the bcrypt hash is never stored in a ConfigMap or any manifest tracked in Git.
