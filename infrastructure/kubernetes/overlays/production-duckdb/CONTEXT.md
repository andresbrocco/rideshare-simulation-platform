...
- **Trino password hashes injected via ExternalSecret**: `TRINO_ADMIN_PASSWORD_HASH` and `TRINO_VISITOR_PASSWORD_HASH` are pulled from AWS Secrets Manager paths `rideshare/trino-admin-password-hash` and `rideshare/trino-visitor-password-hash` and synced into the `app-credentials` Kubernetes Secret by ESO (defined in `../../manifests/external-secrets-app-credentials.yaml`). The `setup-config` initContainer in the Trino pod reads these env vars at startup and renders `password.db` from `password.db.template` via `sed` substitution. The rendered file is written to a shared `emptyDir` volume with `chmod 600` — the bcrypt hashes are never stored in a ConfigMap or any manifest tracked in Git.
