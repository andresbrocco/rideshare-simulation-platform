...
- **Trino `setup-config` initContainer**: The `setup-config` initContainer (image: `busybox:1.36`) copies Trino config files into a shared `emptyDir` volume and generates a shared secret used for inter-service communication. Trino uses header-based identity (`X-Trino-User`) with file-based catalog ACL (`rules.json`) — no password authentication or bcrypt hash computation is performed.
