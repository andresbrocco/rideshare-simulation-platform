...
- AWS Secrets Manager paths `rideshare/trino-admin-password-hash` and `rideshare/trino-visitor-password-hash` populated — required by both overlays for Trino FILE-based authentication (hashes are synced into the `app-credentials` Kubernetes Secret via ESO and expanded into `password.db` at startup by the `setup-config` initContainer)
