...
**Trino FILE-based authentication and access control**: Trino uses `http-server.authentication.type=PASSWORD` with a `file` password authenticator. The `password.db` file is generated at startup by the `setup-config` initContainer, which uses `sed` to substitute `TRINO_ADMIN_PASSWORD_HASH` and `TRINO_VISITOR_PASSWORD_HASH` into a template (`password.db.template`). Access control is enforced via a `rules.json` file: the `admin` user gets full catalog access; all other users get read-only access to the `delta` catalog and no access to the `system` catalog. Query activity is captured via `event-listener.properties` (name=`query-event-listener`) which logs completed queries.

**Grafana Admin dashboard folder**: A dedicated `Admin` folder is provisioned in Grafana (path `/etc/dashboards/admin`) for operator-only dashboards. The `grafana-dashboards-admin` ConfigMap contains the `visitor-activity.json` dashboard, which aggregates per-service access timestamps from multiple sources: Airflow login history (via an `airflow-postgres` PostgreSQL datasource querying the `ab_user` table) and cross-service audit events from Loki. The `airflow-postgres` datasource (uid: `airflow-postgres`) must be separately provisioned.
...
- Trino's `password.db` is **never** stored directly in a ConfigMap because it contains bcrypt password hashes. It is written to the shared `emptyDir` volume by the `setup-config` initContainer using a `chmod 600` template-expand step. The two source hashes (`TRINO_ADMIN_PASSWORD_HASH`, `TRINO_VISITOR_PASSWORD_HASH`) come from separate AWS Secrets Manager paths (`rideshare/trino-admin-password-hash` and `rideshare/trino-visitor-password-hash`) and are synced into the `app-credentials` Kubernetes Secret by ESO.
- The Grafana `visitor-activity.json` dashboard references the `airflow-postgres` datasource by UID. If that datasource is not provisioned, Airflow login panels will fail silently with "Data source not found." The datasource must connect to the `airflow-postgres` Service on port 5432.
