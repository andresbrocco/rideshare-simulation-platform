...
**Trino FILE-based authentication and access control**: Trino uses `http-server.authentication.type=PASSWORD` with a `file` password authenticator. The `password.db` file is generated at startup by the `setup-config` initContainer (image: `httpd:2.4-alpine`), which computes the admin bcrypt hash from `ADMIN_PASSWORD` via `htpasswd`. Only the `admin` account is defined. Access control is enforced via a `rules.json` file: the `admin` user gets full catalog access; all other users get read-only access to the `delta` catalog and no access to the `system` catalog. Query activity is captured via `event-listener.properties` (name=`query-event-listener`) which logs completed queries.
...
- Trino's `password.db` is **never** stored directly in a ConfigMap because it contains bcrypt password hashes. It is written to the shared `emptyDir` volume by the `setup-config` initContainer (image: `httpd:2.4-alpine`) using `htpasswd` to compute the hash from `ADMIN_PASSWORD` with `chmod 600`.
