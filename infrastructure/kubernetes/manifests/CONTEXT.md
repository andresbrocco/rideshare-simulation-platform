...
**Trino header-based identity and access control**: Trino uses header-based identity (`X-Trino-User`) — no password authentication is required. Access control is enforced via a `rules.json` file: the `admin` user gets full catalog access; all other users get read-only access to the `delta` catalog and no access to the `system` catalog. Query activity is captured via `event-listener.properties` (name=`query-event-listener`) which logs completed queries.
...
- The `setup-config` initContainer copies Trino config files into a shared `emptyDir` volume and generates a shared secret for inter-service communication. No password hashes are computed or stored.
