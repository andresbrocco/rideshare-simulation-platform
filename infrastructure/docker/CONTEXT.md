...
- **Grafana conditionally sources data-pipeline secrets**: The Grafana entrypoint also checks for `/secrets/data-pipeline.env` and sources it when present. This gives Grafana access to data-pipeline credentials (e.g., Trino connection details) without requiring the `data-pipeline` profile to be active. The same conditional-source pattern is used as in the simulation service.
- **`lambda-init` bundles visitor-provisioning scripts**: In addition to deploying the auth Lambda, `lambda-init` now mounts three provisioning modules (`provision_grafana_viewer.py`, `provision_airflow_viewer.py`, `provision_minio_visitor.py`) and a MinIO IAM policy file (`minio-visitor-readonly.json`) into the Lambda package path. It injects service endpoint environment variables (`GRAFANA_URL`, `AIRFLOW_URL`, `MINIO_ENDPOINT`, `MINIO_ACCESS_KEY`, `SIMULATION_API_URL`) so the deployed Lambda can call those services at runtime.
- **`secrets-init` hashes passwords with bcrypt**: `bcrypt` is now installed alongside `boto3` and `mypy-boto3-secretsmanager` so that `seed-secrets.py` can hash visitor passwords before storing them in LocalStack Secrets Manager.
