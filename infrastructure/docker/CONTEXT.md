...
**DBT Runner**: The `DBT_RUNNER` environment variable controls which engine runs DBT transformations. Set to `duckdb` for local development (default) or `glue` for AWS Glue. DuckDB uses `dbt-duckdb` for transformations; Glue uses `dbt-glue` against AWS Glue jobs.
...
- `bronze-init` - Registers Delta tables in the Bronze layer
...
**Memory Limits**: All services have explicit `mem_limit` constraints to prevent resource exhaustion during local development (ranges from 512m for Redis to 2048m for airflow-scheduler).
