...
- **Session time-boxing**: The Lambda auth-deploy function enforces a session deadline with auto-teardown via EventBridge Scheduler. A deploying session older than 60 minutes is auto-deleted only if GitHub API confirms the workflow is not running. A tearing-down session older than 15 minutes is auto-deleted (assumed stale).
