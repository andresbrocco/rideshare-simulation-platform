...
def populated_registry():
    reg = DriverRegistry()
    reg.register_driver("driver1", "available", zone_id="pinheiros", location=(-23.561, -46.682))
    reg.register_driver("driver2", "available", zone_id="pinheiros", location=(-23.562, -46.683))
    reg.register_driver(
        "driver3", "en_route_pickup", zone_id="pinheiros", location=(-23.563, -46.684)
    )
    reg.register_driver(
        "driver4", "available", zone_id="vila_madalena", location=(-23.545, -46.690)
    )
...
def test_update_driver_status(registry):
    registry.register_driver("driver1", "available", zone_id="pinheiros")
    registry.update_driver_status("driver1", "en_route_pickup")

    assert registry.get_status_count("available") == 0
    assert registry.get_status_count("en_route_pickup") == 1
    assert registry.get_zone_driver_count("pinheiros", "available") == 0
...
def test_unregister_driver(registry):
    registry.register_driver("driver1", "available", zone_id="pinheiros")
    assert registry.get_status_count("available") == 1
...
def test_get_status_count(populated_registry):
    assert populated_registry.get_status_count("available") == 3
    assert populated_registry.get_status_count("en_route_pickup") == 1
    assert populated_registry.get_status_count("offline") == 1
    assert populated_registry.get_status_count("on_trip") == 0


@pytest.mark.unit
def test_get_zone_driver_count(populated_registry):
    assert populated_registry.get_zone_driver_count("pinheiros", "available") == 2
    assert populated_registry.get_zone_driver_count("pinheiros", "en_route_pickup") == 1
    assert populated_registry.get_zone_driver_count("vila_madalena", "available") == 1
...
def test_status_transition_en_route_pickup_to_destination(registry):
    ...
    registry.update_driver_status("driver1", "on_trip")

    assert registry.get_status_count("en_route_pickup") == 0
    assert registry.get_status_count("on_trip") == 1


@pytest.mark.unit
def test_status_transition_en_route_to_online(registry):
    registry.register_driver("driver1", "on_trip", zone_id="pinheiros")
    registry.update_driver_status("driver1", "available")
...
def test_driver_zone_update(registry):
    registry.register_driver("driver1", "available", zone_id="pinheiros")
    assert registry.get_zone_driver_count("pinheiros", "available") == 1

    registry.update_driver_zone("driver1", "vila_madalena")
    assert registry.get_zone_driver_count("pinheiros", "available") == 0
    assert registry.get_zone_driver_count("vila_madalena", "available") == 1
...
def test_update_driver_location(registry):
    registry.register_driver(
        "driver1", "available", zone_id="pinheiros", location=(-23.561, -46.682)
    )
...
def test_available_drivers_excludes_all_non_online_statuses(registry):
    ...
    registry.register_driver("driver4", "on_trip", zone_id="pinheiros")
