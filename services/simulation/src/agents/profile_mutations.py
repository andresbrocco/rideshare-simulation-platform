...
_fake: "FakerType" = create_faker_instance()
...
def mutate_driver_profile(dna: DriverDNA) -> dict[str, Any]:
    ...
    if roll < 0.40:
        ...
        vehicle = _fake.vehicle_br()
        changes["vehicle_make"] = vehicle["make"]
        changes["vehicle_model"] = vehicle["model"]
        changes["vehicle_year"] = vehicle["year"]
        changes["license_plate"] = _fake.license_plate_br()
    elif roll < 0.60:
        # Phone number change
        changes["phone"] = _fake.phone_br_mobile_sp()
    elif roll < 0.75:
        # Email change
        changes["email"] = _fake.email()
    else:
...
def mutate_rider_profile(dna: RiderDNA) -> dict[str, Any]:
    ...
    if roll < 0.50:
        ...
        payment = _fake.payment_method_br()
