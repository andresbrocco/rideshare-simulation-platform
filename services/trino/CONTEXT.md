...
- **FILE-based password authentication**: `password-authenticator.properties` configures the `file` authenticator pointing at `/tmp/trino-etc/password.db`. Two accounts are defined: `admin` (full access) and `visitor` (read-only, enforced via `rules.json`). Password hashes are injected at runtime via `TRINO_ADMIN_PASSWORD_HASH` and `TRINO_VISITOR_PASSWORD_HASH` environment variables.
