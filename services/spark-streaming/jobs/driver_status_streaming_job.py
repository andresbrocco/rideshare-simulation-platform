"""Streaming job for driver-status topic ingestion to Bronze layer."""
...
class DriverStatusStreamingJob(BaseStreamingJob):
    """Streaming job for driver-status topic."""
    ...
    def bronze_table_path(self) -> str:
        return "s3a://rideshare-bronze/bronze_driver_status/"
...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName("DriverStatusStreamingJob").getOrCreate()
    ...
    checkpoint_config = CheckpointConfig(
        checkpoint_path=os.environ.get(
            "CHECKPOINT_PATH", "s3a://rideshare-checkpoints/driver-status/"
        ),
        trigger_interval=os.environ.get("TRIGGER_INTERVAL", "10 seconds"),
    )
    error_handler = ErrorHandler(
        dlq_table_path="s3a://rideshare-bronze/dlq_driver_status/"
    )
