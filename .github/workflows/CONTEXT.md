...
- The `deploy-lambda` workflow has a comment acknowledging that `update-function-code` causes Terraform to detect source-hash drift on the next `plan`. This is accepted as a no-op on the next apply if no source changes occurred.
